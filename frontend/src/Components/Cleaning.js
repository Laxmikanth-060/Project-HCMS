...
function Cleaning() {
  ...
  const sumbitHandler=(e,id)=>{
   ...
   axios.post("http://localhost:3001/warden/view/cleaning",{id})
   .then(data=>{
    // setData(prevData => {
...
const statushandler=(e)=>{
  ...
  axios.post("http://localhost:3001/warden/view/cleaning/status",{Status})
  .then(data=>{
...
const toggleTemp = () => {
  ...
  useEffect(()=>{
    ...
    axios.get("http://localhost:3001/warden/view/cleaning",{})
    .then((res)=>{
      const Data=res.data;
  ...
  <h2 className='text-center text-2xl font-bold mt-7'>Cleaning Complaints</h2>
