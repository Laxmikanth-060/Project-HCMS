...
function Carpenter() {
  ...
  const sumbitHandler=(e,id)=>{
   ...
   axios.post("http://localhost:3001/warden/view/carpenter",{id})
   .then(data=>{
    // setData(prevData => {
...
const statushandler=(e)=>{
  ...
  axios.post("http://localhost:3001/warden/view/carpenter/status",{Status})
  .then(data=>{
...
const toggleTemp = () => {
  ...
  useEffect(()=>{
    ...
    axios.get("http://localhost:3001/warden/view/carpenter",{})
    .then((res)=>{
      const Data=res.data;
  ...
  <h2 className='text-center text-2xl font-bold mt-7'>Carpenter Complaints</h2>
