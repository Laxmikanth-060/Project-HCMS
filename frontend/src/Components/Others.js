...
function Others() {
  ...
  const sumbitHandler = (e, id) => {
    ...
    axios.post("http://localhost:3001/warden/view/Others", { id })
      .then(() => {
        // Assuming the response will reflect the change, refetch data
        fetchData();
      })
  ...
  const statushandler = (e) => {
    ...
    axios.post("http://localhost:3001/warden/view/Others/status", { Status })
      .then(response => {
  ...
  const fetchData = () => {
    axios.get("http://localhost:3001/warden/view/Others", {})
      .then((res) => {
        const Data = res.data;
        setData(Data);
  ...
  return (
    <div>
      <h2 className='text-center text-2xl font-bold mt-7'>Other Complaints</h2>
            ...
            {data.map((complaint, index) => (
              <tr key={index}>
                ...
                <td className='border border-black-200 text-md'>
                  <form onSubmit={(e) => sumbitHandler(e, complaint._id)}>
                    <button className='border border-black-200 px-5 py-4 text-md w-full bg-green-300 hover:bg-green-500 hover:scale-105'>Resolve</button>
                  </form>
                </td>
              </tr>
            ))}
